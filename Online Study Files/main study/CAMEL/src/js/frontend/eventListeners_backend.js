...
$(document).on("mousedown", ".connector, .outer-connector", function (event) {
    ...
    if (event.detail == 2) {
        ...
        if (CAM.currentConnector != null) {
            ...
            var currentMotherNode = CAM.nodes.filter(el => el.id === CAM.currentConnector.source)[0];
            var currentDaughterNode = CAM.nodes.filter(el => el.id === CAM.currentConnector.target)[0];
