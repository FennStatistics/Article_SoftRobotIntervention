...
p_emo <- p_emo + common_theme
p_concepts <- p_concepts + common_theme + theme(legend.position = "none")
p_negative <- p_negative + common_theme + theme(legend.position = "none")
...
anova3 <- extract_anova(fit1_negative, "Number Negative Concepts")
...
# Remove row names and format table for APA7
table_grob <- tableGrob(
  anova_results,
  rows = NULL, # Removes row numbers
  theme = ttheme_default(
    core = list(
      fg_params = list(hjust = 0, x = 0.1, fontsize = 14) # Increase font size
    ),
    colhead = list(
      fg_params = list(fontface = "bold", fontsize = 16) # Increase header font size
    )
  )
)

# Arrange plots and table into a 2x2 grid
combined_plot <- grid.arrange(
  p_emo, p_concepts, p_negative, table_grob,
  ncol = 2, nrow = 2,
  heights = c(1, 1) # if necessary increase height of the second row (for the table)
)
```
...
p <- ggplot(data_long, aes(x = group, y = value, fill = type)) +
  ...
  geom_text(aes(label = round(value, 1)),  # Add value labels
  ...
  labs(title = "Stacked Barplot of Drawn Rigid and Soft Concepts by Robot Type",
       x = "Robot Type (RR or SAR)",
       y = "Average Number of Concepts Drawn per CAM",
       fill = "Type") +
  theme_minimal() +
  scale_fill_manual(values = c("rigid" = "darkblue", "soft" = "lightblue")) +
  theme(legend.position = "top",
        ...
        axis.title.x = element_text(size = 12),
        axis.title.y = element_text(size = 12),
        strip.text = element_text(size = 14, face = "bold"),  # Increase font size of category names
        panel.spacing = unit(0.5, "lines"))  # Reduce spacing between facets

print(p)

ggsave(filename = "R2_G1.pdf")
ggsave(filename = "R2_G1.jpeg")
```
