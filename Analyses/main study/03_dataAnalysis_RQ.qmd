...
for (c in 1:length(ids_categories)) {
  ...
  dat$CAMs <- round(x = dat$CAMs / num_RR * 100, digits = 2)
dat$Nnew <- round(x = dat$Nnew / num_RR * 1, digits = 2)
dat$Nold <- round(x = dat$Nold / num_RR * 1, digits = 2)
dat$Nconstant <- round(x = dat$Nconstant / num_RR * 1, digits = 2)
