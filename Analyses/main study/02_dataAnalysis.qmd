...
DT::datatable(questionnaireCAMs[,c("choosen_Robot", "meanDifferencesCAMs", "adaptiveQuestion", "feedback_critic")], options = list(pageLength = 5)) 
...
DT::datatable(tmp[,c("choosen_Robot", "meanDifferencesCAMs", "adaptiveQuestion", "feedback_critic")], options = list(pageLength = 5)) 
...
create subset of data to summarize by an LLM:

```{r}
tmp <- tmp[,c("choosen_Robot", "meanDifferencesCAMs", "adaptiveQuestion")]

# dummy - one SD above, below mean
tmp$dummy <- 0
tmp$dummy[tmp$meanDifferencesCAMs > (mean(tmp$meanDifferencesCAMs) + sd(tmp$meanDifferencesCAMs))] <- 1
tmp$dummy[tmp$meanDifferencesCAMs < (mean(tmp$meanDifferencesCAMs) - sd(tmp$meanDifferencesCAMs))] <- -1


setwd("outputs/02_dataAnalysis")
xlsx::write.xlsx2(x = tmp, file = "adaptiveQuestions.xlsx")
```
...
ids_female <- c(questionnaireCAMs$participantCAMpre[questionnaireCAMs$socio_sex == "Female"], 
                questionnaireCAMs$participantCAMpost[questionnaireCAMs$socio_sex == "Female"])

tmp_nodes <- tmp_nodes[tmp_nodes$participantCAM %in% ids_female, ]



tmp_nodes$text[str_detect(string = tmp_nodes$participantCAM, pattern = "_pre$") & tmp_nodes$text_summarized_noSuffix == "HRIP"]
tmp_nodes$text[str_detect(string = tmp_nodes$participantCAM, pattern = "_post$") & tmp_nodes$text_summarized_noSuffix == "HRIP"]


tmp <- tmp_nodes$comment[str_detect(string = tmp_nodes$participantCAM, pattern = "_pre$") & tmp_nodes$text_summarized_noSuffix == "MT"]
tmp[tmp != ""]
```
