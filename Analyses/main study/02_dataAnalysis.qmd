...
for(i in 1:nrow(dat_ids)){
 ...
 %in% dat_ids$PROLIFIC_PID[i] & CAMfiles_combined[[1]]$CAM %in% dat_ids$CAMpre[i],]

 tmp_post <- CAMfiles_combined[[1]][str_remove_all(string = CAMfiles_combined[[1]]$participantCAM, pattern = "_pre$|_post$") %in% dat_ids$PROLIFIC_PID[i] & CAMfiles_combined[[1]]$CAM %in% dat_ids$CAMpost[i],]
...
which(questionnaireCAMs$socio_age %in% tmp)[1:10]
# youngest
