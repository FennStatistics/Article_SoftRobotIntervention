...
dat_translation <- xlsx::read.xlsx2(file = "drawnConcepts_translated_CBJF.xlsx", sheetIndex = 1)
...
Check descriptive statistics for comments:

```{r}
tmp_comments <- sapply(gregexpr("[[:alpha:]]+", CAMfiles_combined[[1]]$comment), function(x) sum(x > 0))
tmp_comments <- tmp_comments[tmp_comments != 0]
psych::describe(tmp_comments)
```

Overwrite German written text and comments to English:
...
Overwrite pre-defined concepts and fix again the following errors:

- multiple times pre-defined concepts
- changed pre-defined concepts because of technical errors


```{r}
## summarized words (suffixes removed):
sort(table(str_remove(string = nodes_translated$text_summarized, pattern = "_positive$|_negative$|_neutral$|_ambivalent$")), decreasing = TRUE)[1:20]

# German words for robots in English abbreviation:
nodes_translated$text_summarized <- str_replace(string = nodes_translated$text_summarized, pattern = "^Rettungsroboter", replacement = "RR")
nodes_translated$text_summarized <- str_replace(string = nodes_translated$text_summarized, pattern = "^Soziale Assistenzroboter", replacement = "SAR")

# German words for risks / benefits to English:
nodes_translated$text_summarized <- str_replace(string = nodes_translated$text_summarized, pattern = "^Nachteile", replacement = "risks")
nodes_translated$text_summarized <- str_replace(string = nodes_translated$text_summarized, pattern = "^Vorteile", replacement = "benefits")

## check summarized words (suffixes removed):
sort(table(str_remove(string = nodes_translated$text_summarized, pattern = "_positive$|_negative$|_neutral$|_ambivalent$")), decreasing = TRUE)[1:20]



## translated words:
sort(table(nodes_translated$text), decreasing = TRUE)[1:10]

# in some cases additional "s"
nodes_translated$text[str_detect(string = nodes_translated$text_summarized, pattern = "^SAR")] <- "social assistance robot"
# manually recode one word, which else would be appear two times as a pre-defined word:
nodes_translated$text[str_detect(string = nodes_translated$text, pattern = "^risks")] <- "low risks"
nodes_translated$text[str_detect(string = nodes_translated$text_summarized, pattern = "^risks")] <- "risks"

nodes_translated$text[str_detect(string = nodes_translated$text_summarized, pattern = "^benefits")] <- "benefits"
